...
vi.mock('frimousse', async () => {
	...
	function usePickerContext() {
		...
		if (!value) {
			throw new Error('Mock EmojiPicker components must be rendered under Root');
		}
		return value;
	}
	...
	function Root({
		...
		const [activeEmoji, setActiveEmoji] = React.useState<MockEmoji | null>(null);
	...
	const Viewport = React.forwardRef<HTMLDivElement, React.ComponentProps<'div'>>(function Viewport(
		{ children, ...props },
		ref,
	) {
		return (
	...
	function List({
		...
		components: {
			CategoryHeader: (props: { category: { label: string } }) => React.ReactNode;
		...
		const groups = filteredItems.reduce<Record<string, MockEmoji[]>>((acc, item) => {
...
describe('EmojiPickerPanel', () => {
	it('shows the search input and empty state for unmatched queries', () => {
		render(<EmojiPickerPanel skinTone="none" onSkinToneChange={vi.fn()} onEmojiSelect={vi.fn()} />);
	...
	it('updates the preview on hover and emits selections', () => {
		...
		render(
			<EmojiPickerPanel skinTone="none" onSkinToneChange={vi.fn()} onEmojiSelect={onEmojiSelect} />,
		);
